...
class AnchorTarget:
    ...
    def __call__(self, target, size, neg=False):
        np.random.seed(233)# for vot
        # np.random.seed(123)# otb
        # np.random.seed(666)  # otb3
        ...
        if len(pos)>16:
            print('p_number:{}'.format(len(pos)))

        # cls[pos] = 3
        # cls[pos] = 0.5
        # cls[neg] = -2

        not_used = np.where(overlap >= cfg.TRAIN.THR_LOW)
        cls[not_used] = 0.5 # not used pos samples

        pos, pos_num = select(pos, cfg.TRAIN.POS_NUM)
        neg, neg_num = select(neg, cfg.TRAIN.TOTAL_NUM - cfg.TRAIN.POS_NUM)
        # print('sample positive number:{}, negative number:{}'.format(pos_num,neg_num))
        cls[pos] = 1
