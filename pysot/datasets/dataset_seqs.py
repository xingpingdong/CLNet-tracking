...
from pysot.datasets.anchor_target import AnchorTarget, Anchors
...
class TrkDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        i = 0
        cls_max = 0
        while i < cfg.LATENTS.NUM_PER_SEQ:
        # for i in range(cfg.LATENTS.NUM_PER_SEQ):
            # get one dataset
            if neg:
            ...
            search, bbox = self.search_aug(search_image,
                                           ...
                                           gray=gray)
            # cv2.imwrite('template.jpg', np.uint8(template))
            # cv2.imwrite('search.jpg', np.uint8(search))
            # get labels
            ...
            cs.append(cls)
            ds.append(delta)
            dws.append(delta_weight)
            bs.append(np.array(bbox))
            i += 1
            cls_max = max(cls_max, cls.max())
            if i == cfg.LATENTS.NUM_PER_SEQ and cls_max < 1:
                i = 0
                index = index0 - 1
                index0 = index
                if index < 0:
                    index = index0  + np.random.randint(2,10)
                    index0 = index
                index = self.pick[index]
                dataset, index = self._find_dataset(index)
                ts, ss, cs, ds, dws, bs = [], [], [], [], [], []
    ...
    def test_get_training_data(self, data, bbox):
        ...
        anchors = self.generate_anchor(25)
        loc = torch.tensor(data['label_loc'])
        pred_bbox = self._convert_bbox(loc, anchors)
        print(pred_bbox)
        pscore = torch.tensor(data['label_cls']).view(-1)
